...
export class UserService {
  ...
  private async saveUser(dto: Partial<User>, role: DefaultRolesEnum) {
    ...
    if(!roleEntity) {
      throw new DomainException({code: HttpStatus.BAD_REQUEST, message: `Невозможно создать пользователя, роль ${role} не найдена`})
    }
  ...
  async findUserByIdOrEmail(where: FindOptionsWhere<User>, withPassword = false): Promise<UserDto> {
    ...
    if(!user) {
      UserException.throwNotFound(where.user_id ? 'user_id' : 'email', where.user_id ? where.user_id as number : where.email as string)
    }
    if(!withPassword) {
